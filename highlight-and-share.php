...
class Highlight_And_Share {
	...
	public function add_footer_html() {
		...
		if ( $settings['show_email'] ) {
			if ( ! $settings['icons'] ) {
				// Note, you must be on a publicly accesible URL to use this button
				$string          = '<div class="has_email" style="display: none;" data-type="email" data-title="%title%" data-url="%url%"><a href="' . esc_html( $settings['ajax_url'] ) . '" target="_blank"><svg class="has-icon"><use xlink:href="#has-email-icon"></use></svg>&nbsp;' . esc_html( apply_filters( 'has_email_text', _x( 'E-mail', 'E-mail share text', 'highlight-and-share' ) ) ) . '</a></div>';
				$html           .= $string;
				$click_to_share .= $string;
				$inline_share   .= $string;
			} else {
				$string          = '<div class="has_email" style="display: none;" data-type="email" data-title="%title%" data-url="%url%"><a href="' . esc_url( $settings['ajax_url'] ) . '" target="_blank"><svg class="has-icon"><use xlink:href="#has-email-icon"></use></svg><span class="has-text">&nbsp;' . esc_html( apply_filters( 'has_email_text', _x( 'E-mail', 'E-mail share text', 'highlight-and-share' ) ) ) . '</span></a></div>';
				$html           .= $string;
				$click_to_share .= $string;
				$inline_share   .= $string;
			}
