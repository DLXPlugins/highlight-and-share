...
var PhotoSwipeDynamicCaption = /*#__PURE__*/function () {
  function PhotoSwipeDynamicCaption(lightbox, options) {
    var _this = this;
    _classCallCheck(this, PhotoSwipeDynamicCaption);
    this.options = _objectSpread(_objectSpread({}, defaultOptions), options);
    this.lightbox = lightbox;
    this.lightbox.on('init', function () {
      _this.initPlugin();
    });
  }
  _createClass(PhotoSwipeDynamicCaption, [{
    key: "initPlugin",
    value: function initPlugin() {
      ...
      this.pswp.on('uiRegister', function () {
        _this2.pswp.ui.registerElement({
    ...
    value: function initCaption() {
      ...
      pswp.on('change', function () {
        _this3.updateCaptionHTML();
        _this3.updateCurrentCaptionPosition();

        // make sure caption is displayed after slides are switched
        _this3.showCaption();
      });
      pswp.on('calcSlideSize', function (e) {
        return _this3.onCalcSlideSize(e);
      });

      // hide caption if mainscroll is shifted (dragging)
      pswp.on('moveMainScroll', function () {
        if (!_this3.useMobileLayout()) {
          if (_this3.pswp.mainScroll.isShifted()) {
            _this3.hideCaption();
          } else {
            _this3.showCaption();
          }
      ...
      pswp.on('beforeZoomTo', function (e) {
    ...
    key: "useMobileLayout",
    value: function useMobileLayout() {
      var mobileLayoutBreakpoint = this.options.mobileLayoutBreakpoint;
    ...
    value: function hideCaption() {
      ...
      if (!this.isCaptionHidden) {
        ...
        this.captionFadeTimeout = setTimeout(function () {
          _this4.captionElement.style.visibility = 'hidden';
          _this4.captionFadeTimeout = null;
        }, 400);
      }
    }
  }, {
    ...
    value: function showCaption() {
      ...
      if (this.isCaptionHidden) {
        ...
        clearTimeout(this.captionFadeTimeout);
        this.captionFadeTimeout = setTimeout(function () {
          _this5.captionElement.classList.remove('pswp__dynamic-caption--faded');
          _this5.captionFadeTimeout = null;
        }, 50);
      }
    }
  }, {
    key: "setCaptionPosition",
    value: function setCaptionPosition(x, y) {
      var isOnHorizontalEdge = x <= this.options.horizontalEdgeThreshold;
      this.captionElement.classList[isOnHorizontalEdge ? 'add' : 'remove']('pswp__dynamic-caption--on-hor-edge');
      this.captionElement.style.left = x + 'px';
      this.captionElement.style.top = y + 'px';
    }
  }, {
    key: "setCaptionWidth",
    value: function setCaptionWidth(captionEl, width) {
      if (!width) {
    ...
    key: "setCaptionType",
    value: function setCaptionType(captionEl, type) {
      var prevType = captionEl.dataset.pswpCaptionType;
      if (type !== prevType) {
    ...
    key: "updateCurrentCaptionPosition",
    value: function updateCurrentCaptionPosition() {
      ...
      var zoomLevel = slide.zoomLevels.initial;
      var imageWidth = Math.ceil(slide.width * zoomLevel);
      var imageHeight = Math.ceil(slide.height * zoomLevel);
      this.setCaptionType(this.captionElement, slide.dynamicCaptionType);
      if (slide.dynamicCaptionType === 'aside') {
        this.setCaptionPosition(this.pswp.currSlide.bounds.center.x + imageWidth, this.pswp.currSlide.bounds.center.y);
        this.setCaptionWidth(this.captionElement, false);
      } else if (slide.dynamicCaptionType === 'below') {
        this.setCaptionPosition(this.pswp.currSlide.bounds.center.x, this.pswp.currSlide.bounds.center.y + imageHeight);
    ...
    key: "createTemporaryCaption",
    value: function createTemporaryCaption() {
    ...
    key: "onCalcSlideSize",
    value: function onCalcSlideSize(e) {
      var slide = e.slide;
      var captionHTML = this.getCaptionHTML(e.slide);
      ...
      var imageWidth = Math.ceil(slide.width * slide.zoomLevels.initial);
      var imageHeight = Math.ceil(slide.height * slide.zoomLevels.initial);
      if (!this.tempCaption) {
      ...
      if (slide.dynamicCaptionType === 'aside') {
        ...
        var captionWidth = captionSize.x;
        var horizontalEnding = imageWidth + slide.bounds.center.x;
        var horizontalLeftover = slide.panAreaSize.x - horizontalEnding;
        ...
        this.setCaptionWidth(this.tempCaption, useMobileVersion ? this.pswp.viewportSize.x : imageWidth);
        this.tempCaption.innerHTML = this.getCaptionHTML(e.slide);
        captionSize = this.measureCaptionSize(this.tempCaption, e.slide);
        var captionHeight = captionSize.y;

        // vertical ending of the image
        var verticalEnding = imageHeight + slide.bounds.center.y;

        // height between bottom of the screen and ending of the image
        // (before any adjustments applied) 
        var verticalLeftover = slide.panAreaSize.y - verticalEnding;
        var initialPanAreaHeight = slide.panAreaSize.y;
        if (verticalLeftover <= captionHeight) {
          ...
          this.recalculateZoomLevelAndBounds(slide);
          var maxPositionX = slide.panAreaSize.x * this.options.mobileCaptionOverlapRatio / 2;
    ...
    key: "measureCaptionSize",
    value: function measureCaptionSize(captionEl, slide) {
      var rect = captionEl.getBoundingClientRect();
      var event = this.pswp.dispatch('dynamicCaptionMeasureSize', {
        captionEl: captionEl,
        slide: slide,
        captionSize: {
          x: rect.width,
    ...
    key: "recalculateZoomLevelAndBounds",
    value: function recalculateZoomLevelAndBounds(slide) {
      slide.zoomLevels.update(slide.width, slide.height, slide.panAreaSize);
      slide.bounds.update(slide.zoomLevels.initial);
    }
  }, {
    key: "storeAdjustedPanAreaSize",
    value: function storeAdjustedPanAreaSize(slide) {
    ...
    key: "storeOriginalPanAreaSize",
    value: function storeOriginalPanAreaSize(slide) {
    ...
    key: "getCaptionHTML",
    value: function getCaptionHTML(slide) {
      if (typeof this.options.captionContent === 'function') {
        return this.options.captionContent.call(this, slide);
      }
      var currSlideElement = slide.data.element;
      var captionHTML = '';
      if (currSlideElement) {
        var hiddenCaption = currSlideElement.querySelector(this.options.captionContent);
        if (hiddenCaption) {
          // get caption from element with class pswp-caption-content
          captionHTML = hiddenCaption.innerHTML;
        } else {
          var img = currSlideElement.querySelector('img');
          if (img) {
    ...
    key: "updateCaptionHTML",
    value: function updateCaptionHTML() {
      var captionHTML = this.getCaptionHTML(pswp.currSlide);
...
/* harmony import */ var _photoswipe_caption__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./photoswipe-caption */ "./src/photoswipe-caption.js");
/* harmony import */ var _photoswipe_caption_css__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./photoswipe-caption.css */ "./src/photoswipe-caption.css");
...
var captionPlugin = new _photoswipe_caption__WEBPACK_IMPORTED_MODULE_2__["default"](lightbox, {
