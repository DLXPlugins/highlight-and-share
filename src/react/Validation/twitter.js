...
const twttr = ( function() {
	...
	function regexSupplant( regex, flags ) {
		...
		return new RegExp(
			regex.replace( /#\{(\w+)\}/g, function( match, name ) {
				let newRegex = twttr.txt.regexen[ name ] || '';
	...
	function addCharsToCharClass( charClass, start, end ) {
		let s = String.fromCharCode( start );
		if ( end !== start ) {
	...
	twttr.txt.linkToTextWithSymbol = function(
		...
		const taggedSymbol = options.symbolTag
	...
	twttr.txt.linkToHashtag = function( entity, text, options ) {
		...
		const hashtag = twttr.txt.htmlEscape( entity.hashtag );
	...
	twttr.txt.linkToCashtag = function( entity, text, options ) {
		const cashtag = twttr.txt.htmlEscape( entity.cashtag );
		const attrs = clone( options.htmlAttrs || {} );
	...
	twttr.txt.linkToMentionAndList = function( entity, text, options ) {
		...
		const user = twttr.txt.htmlEscape( entity.screenName );
		const slashListname = twttr.txt.htmlEscape( entity.listSlug );
	...
	twttr.txt.linkToUrl = function( entity, text, options ) {
		...
		let linkText = twttr.txt.htmlEscape( displayUrl );
	...
	twttr.txt.autoLinkUsernamesOrLists = function( text, options ) {
		const entities = twttr.txt.extractMentionsOrListsWithIndices( text );
		return twttr.txt.autoLinkEntities( text, entities, options );
	};

	twttr.txt.autoLinkHashtags = function( text, options ) {
		const entities = twttr.txt.extractHashtagsWithIndices( text );
		return twttr.txt.autoLinkEntities( text, entities, options );
	};

	twttr.txt.autoLinkCashtags = function( text, options ) {
		const entities = twttr.txt.extractCashtagsWithIndices( text );
		return twttr.txt.autoLinkEntities( text, entities, options );
	};

	twttr.txt.autoLinkUrlsCustom = function( text, options ) {
		const entities = twttr.txt.extractUrlsWithIndices( text, {
			extractUrlsWithoutProtocol: false,
		} );
		return twttr.txt.autoLinkEntities( text, entities, options );
	};
	...
	twttr.txt.extractReplies = function( text ) {
		...
		const possibleScreenName = text.match( twttr.txt.regexen.validReply );
		if (
			! possibleScreenName ||
			...
			if ( ! protocol ) {
				...
				domain.replace(
					...
					function( asciiDomain ) {
						const asciiStartPosition = domain.indexOf(
							asciiDomain,
							asciiEndPosition
						);
	...
	twttr.txt.extractHashtagsWithIndices = function( text, options ) {
		...
		if ( options.checkUrlOverlap ) {
			...
			const urls = twttr.txt.extractUrlsWithIndices( text );
			if ( urls.length > 0 ) {
	...
	twttr.txt.getTweetLength = function( text, options ) {
		...
		let textLength = twttr.txt.getUnicodeTextLength( text ),
	...
	twttr.txt.isValidUsername = function( username ) {
		...
		const extracted = twttr.txt.extractMentions( username );
	...
	twttr.txt.isValidHashtag = function( hashtag ) {
		...
		const extracted = twttr.txt.extractHashtags( hashtag );
	...
	twttr.txt.isValidUrl = function( url, unicodeDomains, requireProtocol ) {
		...
		const urlParts = url.match( twttr.txt.regexen.validateUrlUnencoded );
